...
namespace UGG.Core.Graphics
{
    ...
    static class FontUtil
    {
        ...
        private static Dictionary<FontDesc, Dictionary<char, CharCache>> caches =
            new Dictionary<FontDesc, Dictionary<char, CharCache>>();

        private static List<(Texture2D texture, Rectangle rectangle, Point location)> list =
            new List<(Texture2D texture, Rectangle rectangle, Point location)>(100);
        ...
        public static void DrawStringEx(this SpriteBatch batch, string text, Face font, Color color, int x, int y,
            int width, float depth = 0)
        {
            batch.DrawStringEx(text, font, color, x, y, width, null, FontAlignment.Left, FontVerticalAlignment.Top,
                depth);
        }

        public static void DrawStringEx(this SpriteBatch batch, string text, Face font, Color color, Point position,
            int width, float depth = 0)
        {
            batch.DrawStringEx(text, font, color, position.X, position.Y, width, null, FontAlignment.Left,
                FontVerticalAlignment.Top, depth);
        }

        public static void DrawStringEx(this SpriteBatch batch, string text, Face font, Color color, int x, int y,
            float depth = 0)
        {
            batch.DrawStringEx(text, font, color, x, y, null, null, FontAlignment.Left, FontVerticalAlignment.Top,
                depth);
        }
        ...
        public static void DrawStringEx(this SpriteBatch batch, string text, Face font, Color color, Point position,
            float depth = 0)
        {
            batch.DrawStringEx(text, font, color, position.X, position.Y, null, null, FontAlignment.Left,
                FontVerticalAlignment.Top, depth);
        }

        public static void DrawStringEx(this SpriteBatch batch, string text, Face font, Color color,
            Rectangle rectangle, float depth = 0)
        {
            batch.DrawStringEx(text, font, color, rectangle.X, rectangle.Y, rectangle.Width, rectangle.Height,
                FontAlignment.Left, FontVerticalAlignment.Top, depth);
        }

        static void DrawCounted(SpriteBatch batch, ref CharCache cache, ref Color color, ref int x, ref int y,
            ref int countX1, ref int offsetY1, ref int offsetX1)
        {
            ...
            int count = -1)
        {
            ...
            count = count < 0 ? text.Length : count;
            ...
            {
                var ch = text[i];
                if (!dic2.TryGetValue(ch, out var charCache))
                {
                    continue;
                }

                posX += charCache.Rectangle.Width;
                maxHeight = posY + charCache.Rectangle.Height;

                if (width.HasValue && posX >= width)
                {
                    maxWidth = Math.Max(posX - charCache.Rectangle.Width, maxWidth);
                    posX = 0;
                    posY += charCache.Rectangle.Height;
                    if (height.HasValue && posY >= height - charCache.Rectangle.Height)
                    {
                        break;
                    }
                }
            ...
            return new Point(maxWidth==0? posX:maxWidth, maxHeight);
        }
        ...
        public static void DrawStringEx(
            ...
            float depth = 0)
        {
            batch.DrawStringEx(text, font, color, x, y, width, height, FontAlignment.Left, FontVerticalAlignment.Top,
                depth);
        }
            ...
            float depth = 0)
        {
            ...
            for (var i = 0; i < textLength; i++)
            {
                ...
                if (warpLine && offsetX + cache.Rectangle.Width + countX > width || ch == '\n')
                {
                    if (countX > 0)
                    {
                        DrawCounted(batch, ref headCache, ref color, ref x, ref y, ref countX, ref offsetY,
                            ref offsetX);
                    }
        ...
        private class TextureCache
        {
            ...
            private CharCache AddChar(char ch, Face font, Glyph glyph, BitmapGlyph bitmapGlyph)
            {
                ...
                if ((charIndex > 0 || ch > 255) && !(bitmapGlyph.Bitmap.Width == 0 || bitmapGlyph.Bitmap.Rows == 0))
                {
                    ...
                    var rectangle = new Rectangle(x + cBox.Left, y + (bearingY - cBox.Top), bitmapGlyph.Bitmap.Width,
